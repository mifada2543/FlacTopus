...
import BossBackground from '../components/quiz/BossBackground';
import AiMascot, { AiMascotInner } from '../components/quiz/AiMascot';
import IceBreakingCharacter from '../components/quiz/IceBreakingCharacter';
import IceBreakingView from '../components/quiz/IceBreakingView';
import QuizGameOver from '../components/quiz/QuizGameOver';
import QuizVictory from '../components/quiz/QuizVictory';
import { StudentHPBar, BossHPBar } from '../components/quiz/QuizHPBar';
...
export default function Quiz() {
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.hidden && !isVictory && !isGameOver) {
  ...
  const handleSubmit = async () => {
    ...
    if (selectedAnswer === currentItem.quiz.correctAnswer) {
      ...
      if (currentItem.quiz.type === 'auditory') {
        ...
        const responseText = await getSocraticFeedback(currentItem.quiz.question, selectedAnswer, currentItem.quiz.aiPromptContext, csrfToken);
  ...
  const handleInlineChatSend = async (text) => {
    ...
    try {
      ...
      const aiResponse = await chatWithStudentAssistant(newMessages, JSON.stringify(ctx), user?.nama || "Siswa", csrfToken);
      ...
      <div className={`quiz-page ${(isShaking || isBossDying) ? "shake-screen" : ""}`} style={{ background: flashColor || 'transparent', transition: 'background 0.2s ease-out', padding: 0, pointerEvents: isBossFightNode ? 'none' : 'auto' }}>
        ...
        <div className={`quiz-content-wrapper ${isBossFightNode ? 'boss-wrapper' : ''}`}>
            ...
            >
              ...
              {/* Tab Switch Warning */}
              {tabSwitchCount > 0 && !isMateri && (
                <div style={{ background: 'rgba(239, 68, 68, 0.1)', border: '1px solid rgba(239, 68, 68, 0.3)', borderRadius: '10px', padding: '0.6rem 1rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.85rem', color: '#f87171' }}>
                  <span style={{ fontSize: '1.1rem' }}>⚠️</span>
                  <span>Pindah tab terdeteksi <strong>({tabSwitchCount}x)</strong> — aktivitas ini dicatat.</span>
                </div>
              )}
